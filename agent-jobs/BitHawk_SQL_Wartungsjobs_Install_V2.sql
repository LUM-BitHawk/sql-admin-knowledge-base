...
================================================================================
 ...
 SQL Server – Komplettes Installations-Paket für SLA Kunden ohne VEEAM Plugin
================================================================================
 Datei    : BitHawk_SQL_Wartungsjobs_Install_V2.sql
 Version  : 2.0
 Erstellt : 2026-04-07
 Autor    : BitHawk AG
 Ersteller: Marcel Luginbühl
  ...
  Inhalt (Reihenfolge):
   ...
   PHASE 6  – Job 1  BitHawk_Backup_Full_Wochenende          (Sa 00:00)
   PHASE 7  – Job 2  BitHawk_Integrity_Check_Samstag         (Sa 04:00)
   PHASE 8  – Job 3  BitHawk_Index_Rebuild_Samstag           (Sa 08:00)
   PHASE 9  – Job 4  BitHawk_Statistics_Update_Samstag       (Sa 10:00)
   PHASE 10 – Job 5  BitHawk_QueryStore_Bericht_Samstag      (Sa 11:00)
   PHASE 11 – Job 6  BitHawk_Backup_Cleanup_Samstag          (Sa 12:00)
   PHASE 12 – Job 7  BitHawk_Backup_Diff_Sonntag             (So 00:00)
   PHASE 13 – Job 8  BitHawk_Backup_TLog_Stuendlich_Werktags (Mo-Fr 06-22h)
   PHASE 14 – Job 9  BitHawk_Backup_TLog_Cleanup_Taeglich    (tgl. 23:00)
   PHASE 15 – Verifikation aller Jobs und Benutzer
   ANHANG A – Query Store aktivieren (auskommentiert)
   ANHANG B – Deinstallation (auskommentiert)

 Backup-Pfad : S:\SQLBackup\Full\  S:\SQLBackup\Diff\  S:\SQLBackup\Log\
 Job-Owner   : BHSQLJobRun (SQL Authentication, sysadmin)
...
-- ============================================================================
-- PHASE 6: Job 1 – BitHawk_Backup_Full_Wochenende  (Samstag 00:00)
-- ============================================================================
PRINT '--- PHASE 6: Job 1/9 – BitHawk_Backup_Full_Wochenende -----------------';
GO

USE [msdb];
GO

IF EXISTS (SELECT 1 FROM msdb.dbo.sysjobs WHERE name = N'BitHawk_Backup_Full_Wochenende')
BEGIN
    EXEC sp_delete_job @job_name = N'BitHawk_Backup_Full_Wochenende', @delete_unused_schedule = 1;
    PRINT '       -> Bestehender Job entfernt.';
END

EXEC sp_add_job
    @job_name              = N'BitHawk_Backup_Full_Wochenende',
    @description           = N'Vollstaendiges komprimiertes Backup aller Online-Datenbanken auf S:\SQLBackup\Full\.',
    @enabled               = 1,
...
EXEC sp_add_jobstep
    @job_name          = N'BitHawk_Backup_Full_Wochenende',
    @step_name         = N'Full Backup alle User-DBs',
    @subsystem         = N'TSQL',
    @on_success_action = 1,
    @on_fail_action    = 2,
    @command           = N'
...
BEGIN
    SET @Path = ''S:\SQLBackup\Full\'' + @DB + ''_Full_'' + @Date + ''.bak'';
...
EXEC sp_add_schedule
    @schedule_name          = N'BitHawk_Sched_Samstag_0000',
    @freq_type              = 8,
    @freq_interval          = 64,     -- Samstag
...
EXEC sp_add_jobserver @job_name = N'BitHawk_Backup_Full_Wochenende';

PRINT '[OK]   Job 1 installiert: BitHawk_Backup_Full_Wochenende';
PRINT '';
GO

-- ============================================================================
-- PHASE 7: Job 2 – BitHawk_Integrity_Check_Samstag  (Samstag 04:00)
-- ============================================================================
...
-- ============================================================================
-- PHASE 8: Job 3 – BitHawk_Index_Rebuild_Samstag  (Samstag 08:00)
-- ============================================================================
PRINT '--- PHASE 8: Job 3/9 – BitHawk_Index_Rebuild_Samstag ------------------';
GO

USE [msdb];
GO

IF EXISTS (SELECT 1 FROM msdb.dbo.sysjobs WHERE name = N'BitHawk_Index_Rebuild_Samstag')
BEGIN
    EXEC sp_delete_job @job_name = N'BitHawk_Index_Rebuild_Samstag', @delete_unused_schedule = 1;
    PRINT '       -> Bestehender Job entfernt.';
END

EXEC sp_add_job
    @job_name              = N'BitHawk_Index_Rebuild_Samstag',
    @description           = N'Rebuild aller Indizes mit Fragmentierung > 30%% (ONLINE=ON, SORT_IN_TEMPDB=ON) auf allen Benutzerdatenbanken.',
    @enabled               = 1,
    @owner_login_name      = N'BHSQLJobRun',
    @notify_level_eventlog = 2;
...
EXEC sp_add_jobstep
    @job_name          = N'BitHawk_Index_Rebuild_Samstag',
    @step_name         = N'Index Rebuild >30%% alle DBs',
    @subsystem         = N'TSQL',
    @on_success_action = 1,
    @on_fail_action    = 2,
    @command           = N'
...
BEGIN
    ...
    DECLARE @Sch NVARCHAR(128), @Tbl NVARCHAR(256), @Idx NVARCHAR(256);
    ...
    OPEN idx_cursor;
    FETCH NEXT FROM idx_cursor INTO @Sch, @Tbl, @Idx;
    WHILE @@FETCH_STATUS = 0
    BEGIN
        EXEC(''''ALTER INDEX '''' + QUOTENAME(@Idx)
           + '''' ON ''''
...
EXEC sp_add_schedule
    @schedule_name          = N'BitHawk_Sched_Samstag_0800',
    @freq_type              = 8,
    @freq_interval          = 64,
    @freq_recurrence_factor = 1,
...
EXEC sp_add_jobserver @job_name = N'BitHawk_Index_Rebuild_Samstag';

PRINT '[OK]   Job 3 installiert: BitHawk_Index_Rebuild_Samstag';
PRINT '';
GO

-- ============================================================================
-- PHASE 9: Job 4 – BitHawk_Statistics_Update_Samstag  (Samstag 10:00)
-- ============================================================================
...
-- ============================================================================
-- PHASE 10: Job 5 – BitHawk_QueryStore_Bericht_Samstag  (Samstag 11:00)
-- ============================================================================
...
-- ============================================================================
-- PHASE 11: Job 6 – BitHawk_Backup_Cleanup_Samstag  (Samstag 12:00)
-- ============================================================================
PRINT '--- PHASE 11: Job 6/9 – BitHawk_Backup_Cleanup_Samstag ----------------';
GO

USE [msdb];
GO

IF EXISTS (SELECT 1 FROM msdb.dbo.sysjobs WHERE name = N'BitHawk_Backup_Cleanup_Samstag')
BEGIN
    EXEC sp_delete_job @job_name = N'BitHawk_Backup_Cleanup_Samstag', @delete_unused_schedule = 1;
    PRINT '       -> Bestehender Job entfernt.';
END

EXEC sp_add_job
    @job_name              = N'BitHawk_Backup_Cleanup_Samstag',
    @description           = N'Loescht abgelaufene Backups: Full > 14 Tage, Diff > 7 Tage, TLog > 7 Tage von S:\SQLBackup\.',
    @enabled               = 1,
    @owner_login_name      = N'BHSQLJobRun',
    @notify_level_eventlog = 2;
...
EXEC sp_add_jobstep
    @job_name          = N'BitHawk_Backup_Cleanup_Samstag',
    @step_name         = N'Cleanup S:\SQLBackup',
    @subsystem         = N'TSQL',
    @on_success_action = 1,
    @on_fail_action    = 2,
    @command           = N'
-- xp_delete_file erfordert vorberechnete Datumsvariablen (kein DATEADD direkt moeglich)
DECLARE @cutoff14 DATETIME = DATEADD(DAY, -14, GETDATE());
DECLARE @cutoff7  DATETIME = DATEADD(DAY,  -7, GETDATE());
DECLARE @cutoff30 DATETIME = DATEADD(DAY, -30, GETDATE());

-- Full Backups aelter als 14 Tage
EXEC xp_delete_file 0, N''S:\SQLBackup\Full\'', N''bak'', @cutoff14, 1;

-- Differenzielle Backups aelter als 7 Tage
EXEC xp_delete_file 0, N''S:\SQLBackup\Diff\'', N''bak'', @cutoff7, 1;

-- T-Log Backups aelter als 7 Tage
EXEC xp_delete_file 0, N''S:\SQLBackup\Log\'',  N''trn'', @cutoff7, 1;

-- msdb Backup-Historie aelter als 30 Tage
EXEC msdb.dbo.sp_delete_backuphistory
    @oldest_date = @cutoff30;
';

EXEC sp_add_schedule
    @schedule_name          = N'BitHawk_Sched_Samstag_1200',
    @freq_type              = 8,
    @freq_interval          = 64,
    @freq_recurrence_factor = 1,
...
EXEC sp_add_jobserver @job_name = N'BitHawk_Backup_Cleanup_Samstag';

PRINT '[OK]   Job 6 installiert: BitHawk_Backup_Cleanup_Samstag';
PRINT '';
GO

-- ============================================================================
-- PHASE 12: Job 7 – BitHawk_Backup_Diff_Sonntag  (Sonntag 00:00)
-- ============================================================================
PRINT '--- PHASE 12: Job 7/9 – BitHawk_Backup_Diff_Sonntag -------------------';
GO

USE [msdb];
GO

IF EXISTS (SELECT 1 FROM msdb.dbo.sysjobs WHERE name = N'BitHawk_Backup_Diff_Sonntag')
BEGIN
    EXEC sp_delete_job @job_name = N'BitHawk_Backup_Diff_Sonntag', @delete_unused_schedule = 1;
    PRINT '       -> Bestehender Job entfernt.';
END

EXEC sp_add_job
    @job_name              = N'BitHawk_Backup_Diff_Sonntag',
    @description           = N'Differenzielles Backup aller Online-Datenbanken auf S:\SQLBackup\Diff\.',
    @enabled               = 1,
    @owner_login_name      = N'BHSQLJobRun',
    @notify_level_eventlog = 2;
...
EXEC sp_add_jobstep
    @job_name          = N'BitHawk_Backup_Diff_Sonntag',
    @step_name         = N'Diff Backup alle User-DBs',
    @subsystem         = N'TSQL',
    @on_success_action = 1,
    @on_fail_action    = 2,
    @command           = N'
...
WHILE @@FETCH_STATUS = 0
BEGIN
    SET @Path = ''S:\SQLBackup\Diff\'' + @DB + ''_Diff_'' + @Date + ''.bak'';
...
EXEC sp_add_schedule
    @schedule_name          = N'BitHawk_Sched_Sonntag_0000',
    @freq_type              = 8,
...
EXEC sp_add_jobserver @job_name = N'BitHawk_Backup_Diff_Sonntag';

PRINT '[OK]   Job 7 installiert: BitHawk_Backup_Diff_Sonntag';
PRINT '';
GO
...
-- PHASE 13: Job 8 – BitHawk_Backup_TLog_Stuendlich_Werktags  (Mo-Fr 06-22h)
-- ============================================================================
PRINT '--- PHASE 13: Job 8/9 – BitHawk_Backup_TLog_Stuendlich_Werktags -------';
GO

USE [msdb];
GO

IF EXISTS (SELECT 1 FROM msdb.dbo.sysjobs WHERE name = N'BitHawk_Backup_TLog_Stuendlich_Werktags')
BEGIN
    EXEC sp_delete_job @job_name = N'BitHawk_Backup_TLog_Stuendlich_Werktags', @delete_unused_schedule = 1;
    PRINT '       -> Bestehender Job entfernt.';
END

EXEC sp_add_job
    @job_name              = N'BitHawk_Backup_TLog_Stuendlich_Werktags',
    @description           = N'Stuendliches T-Log Backup aller FULL/BULK_LOGGED Datenbanken auf S:\SQLBackup\Log\<DBName>\.',
    @enabled               = 1,
    @owner_login_name      = N'BHSQLJobRun',
    @notify_level_eventlog = 2;
...
EXEC sp_add_jobstep
    @job_name          = N'BitHawk_Backup_TLog_Stuendlich_Werktags',
    @step_name         = N'T-Log Backup alle FULL-Recovery DBs',
    @subsystem         = N'TSQL',
    @on_success_action = 1,
    @on_fail_action    = 2,
    @command           = N'
...
WHILE @@FETCH_STATUS = 0
BEGIN
    SET @Dir = ''S:\SQLBackup\Log\'' + @DB;
    SET @Cmd = ''IF NOT EXIST "'' + @Dir + ''" MD "'' + @Dir + ''">NUL'';
    EXEC xp_cmdshell @Cmd, NO_OUTPUT;

    SET @Path = @Dir + ''\'' + @DB + ''_Log_'' + @Date + ''.trn'';
...
EXEC sp_add_schedule
    @schedule_name          = N'BitHawk_Sched_TLog_Werktags_Stuendlich',
    @freq_type              = 8,
    @freq_interval          = 62,     -- Mo(2)+Di(4)+Mi(8)+Do(16)+Fr(32)
    @freq_subday_type       = 8,      -- Stuendlich
...
EXEC sp_add_jobserver @job_name = N'BitHawk_Backup_TLog_Stuendlich_Werktags';

PRINT '[OK]   Job 8 installiert: BitHawk_Backup_TLog_Stuendlich_Werktags';
PRINT '';
GO
...
-- PHASE 14: Job 9 – BitHawk_Backup_TLog_Cleanup_Taeglich  (tgl. 23:00)
-- ============================================================================
PRINT '--- PHASE 14: Job 9/9 – BitHawk_Backup_TLog_Cleanup_Taeglich ----------';
GO

USE [msdb];
GO

IF EXISTS (SELECT 1 FROM msdb.dbo.sysjobs WHERE name = N'BitHawk_Backup_TLog_Cleanup_Taeglich')
BEGIN
    EXEC sp_delete_job @job_name = N'BitHawk_Backup_TLog_Cleanup_Taeglich', @delete_unused_schedule = 1;
    PRINT '       -> Bestehender Job entfernt.';
END

EXEC sp_add_job
    @job_name              = N'BitHawk_Backup_TLog_Cleanup_Taeglich',
    @description           = N'Loescht T-Log Dateien aelter als 7 Tage aus S:\SQLBackup\Log\ inkl. Unterordner.',
    @enabled               = 1,
    @owner_login_name      = N'BHSQLJobRun',
    @notify_level_eventlog = 2;
...
EXEC sp_add_jobstep
    @job_name          = N'BitHawk_Backup_TLog_Cleanup_Taeglich',
    @step_name         = N'TLog Cleanup S:\SQLBackup\Log > 7 Tage',
    @subsystem         = N'TSQL',
    @on_success_action = 1,
    @on_fail_action    = 2,
    @command           = N'
...
EXEC sp_add_schedule
    @schedule_name          = N'BitHawk_Sched_TLog_Cleanup_2300',
...
EXEC sp_add_jobserver @job_name = N'BitHawk_Backup_TLog_Cleanup_Taeglich';

PRINT '[OK]   Job 9 installiert: BitHawk_Backup_TLog_Cleanup_Taeglich';
PRINT '';
GO
...
-- =============================================================================
-- Ende: BitHawk_SQL_Wartungsjobs_Install_V2.sql  |  Version 2.0  |  BitHawk AG
-- =============================================================================
